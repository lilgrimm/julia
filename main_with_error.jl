...
function exact_rk4_solution(t)
    return r_interp(t)#, v_interp(t)
end
...
for (k, n) in enumerate(n_loop)
    ...
    err_r[k, 1] = norm(exact_rk4_solution.(tspan_n) - r_rk4_n, Inf)
    ...
    err_r[k, 2] = norm(exact_rk4_solution.(tspan_n) - r_simpson_n, Inf)
    ...
    err_r[k, 3] = norm(exact_rk4_solution.(tspan_n) - r_fdeuler_n, Inf)
...
#error vs N
p_error = plot(n_loop, err_r, m = :o, label = [L"RK4" L"Num. Integration: Simpsons" L"Central Finite Difference"])
plot!(p_error, n_loop, 10 * 10 * n_loop .^ (-4); 
    l = (:dash, :black), 
    label = "4th order",
    xaxis = (:log10, "n"),
    yaxis = (:log10, "max error"),
    title = "Convergence of Numarical Techniques",
    size=(800,600))

plot!(p_error, n_loop, 10 * 10 * n_loop .^ (-2); 
    l = (:dash, :gray), 
    label = "2nd order")
