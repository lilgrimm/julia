...
#error vs N (r vector)
p_error_r = plot(n_loop, err_r, m = [:p :v :h], label = ["RK4" "Num. Integration: Simpsons" "Central Finite Difference"], width = 2)
plot!(p_error_r, n_loop, 10 * 10 * n_loop .^ (-4); 
    l = (:dash, :black), 
    label = "4th-Order",
    width = 2,
    xaxis = (:log10, "Number of Points, n"),
    yaxis = (:log10, "Max Error"),
    title = "Position Error From Various Methods",
    size=(800,600))
...
#error vs N (r vector)
p_error_v = plot(n_loop, err_r, m = [:p :v :h], label = ["RK4" "Num. Integration: Simpsons" "Central Finite Difference"], width = 2)
plot!(p_error_v, n_loop, 10 * 10 * n_loop .^ (-4); 
    ...
    xaxis = (:log10, "Number of Points, n"),
    yaxis = (:log10, "Max Error"),
    title = "Velocity Error Resulting From Various Methods",
    size=(800,600))
